//! Data types for source management.
...
impl MissingSource {
    ...
    pub fn new(title: impl Into<String>, card_ids: &[String]) -> Self {
        let card_ids_vec: Vec<String> = card_ids.to_vec();
        let sample_card_ids = card_ids_vec.iter().take(5).cloned().collect();
        Self {
            title: title.into(),
            card_count: card_ids_vec.len(),
            sample_card_ids,
            suggestions: Vec::new(),
        }
    }
...
/// A source configured but missing from filesystem.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MissingFile {
    ...
    /// The source category (oxford, general, papers)
    pub category: String,
}
...
mod tests {
    ...
    fn test_missing_source_sample_limit() {
        ...
        assert_eq!(ms.sample_card_ids.len(), 5); // Limited to 5
    }
