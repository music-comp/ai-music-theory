# A Guide for Parallel Concept Card Re-Extraction (v3.1)
...
- Added Phase 0 Step 0.0: Audit existing cards before planning
- Modified Phase 2 agent prompts with merge-aware re-extraction instructions
- Added Phase 3 preservation checks (card count comparison, content spot-check)
...
- **Existing cards** at `concept-cards/<source-slug>/` (will be overwritten in place)
- **Howto guide** available: reference `0004-concept-card-extraction-howto-v3.1.md` for template and field definitions
...
### Step 0.0: Audit Existing Cards (NEW in v3.1)

Before planning extraction, audit the existing card inventory to understand the gap:

```bash
# Count existing cards
find concept-cards/<source-slug>/ -name '*.md' | wc -l
...
for field in slug subcategory tier source_slug extraction_confidence \
  ...
  missing=$(grep -rL "^${field}:" concept-cards/<source-slug>/ | wc -l)
  total=$(find concept-cards/<source-slug>/ -name '*.md' | wc -l)
...
for section in "# Quick Definition" "# Core Definition" "# Prerequisites" \
  ...
  count=$(grep -rl "$section" concept-cards/<source-slug>/ | wc -l)
  total=$(find concept-cards/<source-slug>/ -name '*.md' | wc -l)
...
Also cross-reference against the existing card inventory in concept-cards/<source-slug>/:
...
- **Howto reference**: `crates/design/dev/skill/0004-concept-card-extraction-howto-v3.1.md`

## Re-Extraction Instructions

This is a RE-EXTRACTION task. Cards already exist at `concept-cards/<SourceSlug>/` but were created with an older template missing required fields and sections.

For each concept you extract:

1. **Extract fresh** from the source chapter using the v3 template below
2. **Check** if `concept-cards/<SourceSlug>/<slug>.md` already exists
...
2. **Source material is primary** — Extract from `sources-md/`, not from old cards
...
old_count=$(find concept-cards/<source-slug>/ -name '*.md' | wc -l)
# (Run after re-extraction)
new_count=$(find concept-cards/<source-slug>/ -name '*.md' | wc -l)
...
5. Save to `extraction-metadata/<source-slug>/`
